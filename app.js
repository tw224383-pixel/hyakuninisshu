// -------------------------------------------------------------
    ...
    const goshikiStorage = safegoshikiStorage;
    ...
    function setPlayMode(mode) {
      playMode = mode;
      document.getElementById('mode-traditional').classList.toggle('active', mode === 'traditional');
    ...
    function renderBoard() {
      ...
      displayPoems.forEach(poem => {
        ...
        card.className = `karuta-card ${cardClass} ${borderStyle}`;
    ...
    function openAchievementsModal() {
      ...
      SECRET_TITLES.forEach(title => {
        const isUnlocked = perfectColors.length >= title.id;
      ...
      specials.forEach(title => {
        const isUnlocked = specialAchievements.includes(title.key);
        const row = document.createElement('div');
        row.className = 'achievement-row';
        row.innerHTML = `
          <div class="achievement-icon ${isUnlocked ? 'unlocked' : ''}" style="font-size: 2rem;">${title.icon}</div>
    ...
    function renderBookGrid() {
      ...
      const acquiredCount = Math.min(100, takenCards.length);
      ...
      filtered.forEach(poem => {
        const isUnlocked = takenCards.includes(poem.no);
        const isRead = readCards.includes(poem.no);
        const card = document.createElement('div');
    ...
    window.submitToRanking = function() {
      ...
      if (typeof window.submitRecordToFirebase === 'function') {
        window.submitRecordToFirebase(name, lastGameElapsed, missesCount, selectedColor)
          .then(() => {
