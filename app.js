window.addEventListener('error', function(e) {
    ...
    window.baseGoshikiStorage = baseGoshikiStorage;
    const goshikiStorage = {
      ...
      removeItem: function(key) {
        ...
        if (!window.isGuestMode && typeof window.syncRemoveFromFirebase === 'function') {
          window.syncRemoveFromFirebase(key);
        }
      },
      clear: function() {
        baseGoshikiStorage.clear();
        if (!window.isGuestMode && typeof window.syncClearFirebase === 'function') {
    ...
    function getLevelStyle(level) {
      if (level >= 500) return { bg: 'linear-gradient(135deg, #fef08a, #fca5a5, #c084fc)', border: '#f59e0b', text: '#78350f' };
      if (level >= 451) return { bg: '#fef3c7', border: '#f59e0b', text: '#78350f' };
      if (level >= 401) return { bg: '#f1f5f9', border: '#cbd5e1', text: '#334155' };
      if (level >= 351) return { bg: '#e0e7ff', border: '#818cf8', text: '#1e1b4b' };
      if (level >= 301) return { bg: '#faf5ff', border: '#d8b4fe', text: '#581c87' };
      if (level >= 251) return { bg: '#ffe4e6', border: '#fecdd3', text: '#881337' };
      if (level >= 201) return { bg: '#ffedd5', border: '#fed7aa', text: '#7c2d12' };
      if (level >= 151) return { bg: '#d1fae5', border: '#a7f3d0', text: '#064e3b' };
      if (level >= 101) return { bg: '#f3e8ff', border: '#e9d5ff', text: '#581c87' };
      if (level >= 51) return { bg: '#e0f2fe', border: '#bae6fd', text: '#0c4a6e' };
      return { bg: '#f8fafc', border: '#e2e8f0', text: '#475569' };
    }
    window.getLevelStyle = getLevelStyle;
    ...
    let isGuestMode = goshikiStorage.getItem('goshiki_is_guest_mode') === 'true';
    window.isGuestMode = isGuestMode;
    let rankingSortKey = 'time'; // 'time' or 'date'
    window.rankingSortKey = rankingSortKey;
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      // Load saved username
      let savedName = goshikiStorage.getItem('goshiki_current_username') || goshikiStorage.getItem('goshiki_ranking_username') || '';
      savedName = savedName.replace(/\s*\(\d+(\.\d+)?秒?\)$/, '').trim();
      ...
      // Render announcements and check version updates
    ...
    // (getLevel is defined globally above)
    ...
    function renderBoard() {
      ...
      displayPoems.forEach(poem => {
        ...
        // Append equipped card effect overlay
        const equippedEffect = goshikiStorage.getItem('goshiki_equipped_effect') || 'none';
        if (equippedEffect !== 'none') {
          const overlay = document.createElement('div');
          overlay.className = `card-effect-overlay effect-${equippedEffect}`;
    ...
    function handleCardTap(cardNo) {
      ...
      if (cardNo === currentTargetPoem.no) {
        ...
        playCorrectSound();
        addXP(10);
        if (typeof addPoints === 'function') addPoints(10, 'correct_card', cardNo.toString());
    ...
    function endGame() {
      ...
      // Reset ranking submission UI based on penalty state & Guest Mode
      ...
      const guestNotice = document.getElementById('ranking-guest-notice');
      ...
      if (window.isGuestMode) {
        ...
        if (hasTriggeredPenalty) {
          ...
          const nameInput = document.getElementById('ranking-name-input');
          if (nameInput) {
            let savedName = goshikiStorage.getItem('goshiki_current_username') || '';
            savedName = savedName.replace(/\s*\(\d+(\.\d+)?秒?\)$/, '').trim();
            nameInput.value = savedName;
            nameInput.readOnly = true;
          }
      ...
      // Submit miss stats to Firebase
      if (!window.isGuestMode && typeof window.submitMissStatsToFirebase === 'function') {
        window.submitMissStatsToFirebase(sessionMisses);
      }
      updateCoachAdvice();
      ...
      // Check weekly stats updates and limited event achievements
      if (typeof window.checkAndUpdateWeeklyHeroes === 'function') {
        window.checkAndUpdateWeeklyHeroes(0, missesCount, currentSet.length);
      }
      if (typeof window.checkEventAchievements === 'function') {
        window.checkEventAchievements(missesCount);
      }
    }
    ...
    const SECRET_TITLES = [
      ...
      { id: 2, name: "二色の賢者", icon: "🌙", req: "いずれか2つの色をノーミスクリア", desc: "集中力を高め、2つの色を完全に制覇した知恵の探求者！" },
    ...
    const NEW_ACHIEVEMENTS = [
      ...
      { key: "mix_master_40", name: "ミックスの使い手（40枚）", icon: "🏹", req: "ミックス40枚コースで初めてランキング入りする", desc: "2つの色が入り乱れる戦場を征し、見事ランキング入りを果たした実力者！", color: "#4f46e5" },
    ...
    // -------------------------------------------------------------
    // Mascot, Date, and Particle Helpers
    // -------------------------------------------------------------
    ...
    let currentMascotTriggerType = null;
    ...
    function checkStartupMascot() {
      const d = new Date();
      const hour = d.getHours();
      if (hour >= 16 && hour < 20) {
        const lastMascotDate = goshikiStorage.getItem('goshiki_last_mascot_date') || '';
        const todayStr = getTodayDateString();
        if (lastMascotDate !== todayStr) {
          if (Math.random() < 0.1) {
            spawnMascot('startup');
          }
        }
    ...
    window.checkStartupMascot = checkStartupMascot;
    ...
    // Guest Mode Controls
    // -------------------------------------------------------------
    function promptGuestConsent() {
      const loginModal = document.getElementById('login-modal');
      if (loginModal) loginModal.style.display = 'none';
      const consentModal = document.getElementById('guest-consent-modal');
      if (consentModal) consentModal.style.display = 'flex';
    }
    window.promptGuestConsent = promptGuestConsent;

    function closeGuestConsent() {
      const consentModal = document.getElementById('guest-consent-modal');
      if (consentModal) consentModal.style.display = 'none';
      const loginModal = document.getElementById('login-modal');
      if (loginModal) loginModal.style.display = 'flex';
    }
    window.closeGuestConsent = closeGuestConsent;

    function confirmGuestModeStart() {
      const consentModal = document.getElementById('guest-consent-modal');
      if (consentModal) consentModal.style.display = 'none';
      
      window.isGuestMode = true;
      goshikiStorage.setItem('goshiki_is_guest_mode', 'true');
      goshikiStorage.setItem('goshiki_current_username', 'ゲスト');
    ...
    window.confirmGuestModeStart = confirmGuestModeStart;

    function promptGuestToRegister() {
      window.isGuestMode = false;
      goshikiStorage.removeItem('goshiki_is_guest_mode');
      ...
      if (statusMsg) {
        ...
        statusMsg.textContent = 'ニックネームと合言葉を入力して、アカウントを作成してください！これまでのレベルや称号がそのまま保存されます。';
      }
    }
    window.promptGuestToRegister = promptGuestToRegister;
    ...
    const APP_UPDATES = [
      {
        version: 'v6.0',
        date: '2026/07/23',
        title: '「じぶんステータス」＆「うさぎハンターランキング」＆「まいにちログイン」が新登場！',
        details: [
          '「じぶんステータス」で、これまでログインした日数、まいにち続けている日数、とったお札の数、今週あつめたXP、もっている称号の数、つかまえたうさちゃんの数がぜんぶ一目でわかるようになったよ！カードにマウスを当てるとふわりと動くよ！',
          'かくれているうさちゃん🐰をタップすると、つかまえた数が増えていくよ！累計で30匹つかまえると、特別な称号「うさぎの友達」がもらえるよ！',
          '今週のヒーローに新しく「うさぎハンターランキング」が追加されたよ！だれが一番たくさんうさちゃんをつかまえられるか、みんなで競争しよう！毎週月曜日の朝にリセットされるよ！',
      ...
      },
      {
        version: 'v5.2',
        date: '2026/07/19',
        title: 'ポイントシステムと「歌詠みの商店街」ショップの開店！',
        details: [
          '修行場で札を正解するたびにポイント（1枚あたり10pt）が貯まるようになりました！',
          '貯まったポイントを使ってカードが光る・揺れるエフェクトを購入できる「歌詠みの商店街（ショップ）」がオープンしました！',
          'エフェクトを実際にカードに当てて動きを確認できる「試着室（プレビュー）」機能も搭載！納得したエフェクトを選んでみよう！'
        ]
      },
      {
        version: 'v5.0',
        date: '2026/07/19',
        title: 'レイアウト中央寄せの調整とランキング名前変更の禁止',
        details: [
          'ゲーム開始画面の設定・ボタン類を端末の画面中央にきれいに整列させ、文字の読みやすさを維持したまま配置を最適化しました！',
          'ランキング送信時の名前は「ログインした登録名」に固定され、手動での変更を行えないように安全に設計しました。'
        ]
      },
      {
        version: 'v4.9',
        date: '2026/07/18',
        title: '画面の自動中央寄せと修行場へのリニューアル',
        details: [
          'パソコン・タブレット・スマートフォンで開いた際に、全体のレイアウトが自動的に画面中央へ美しく整列するよう改善しました！',
          'トップ画面のモード名を「百人一首 練習モード」から、より没入感のある「歌詠みの修行場」へと変更しました。'
        ]
      },
      {
        version: 'v4.8',
        date: '2026/07/18',
        title: '「今週のヒーロー」１位～３位表示の実装と機能追加',
        details: [
          'トップ画面に表示される「今週のヒーロー」が、１位から３位まで並んで表示されるようになりました！',
          '記録がないときでも常に順位が表示され、まだランキングに載っていない週でも挑戦枠として確認しやすくなりました。'
        ]
      },
      {
        version: 'v4.7',
        date: '2026/07/18',
        title: 'レベルに応じた名前の背景色変更と出現確率の微調整',
        details: [
          'プレイヤーのレベル段位に応じて、ヘッダーやお名前、順位表の背景が特別なカラープレートに変化するようになりました！',
          '３枚連続おてつき無しで正解した際の隠れキャラ（うさちゃん）出現率を、５％（20分の1）に変更・微調整しました。'
        ]
      },
      {
        version: 'v4.6',
        date: '2026/07/18',
        title: '順位表の表示改善とレベル・称号バッジ不具合の修正',
        details: [
          '順位表（ランキング）の名前が途中で切れずに、全員 of フルネームが表示されるようになりました！',
          '順位表の登録時に、ユーザーの現在のレベル（Lxx）が正確に送信・表示されない不具合を解消しました。',
          '「ミックスの使い手」と「ミックスの覇者」の称号アイコンが他の称号と重複していた不具合を修正し、それぞれ新しいデザイン（🏹 と 🐲）に変更しました！'
        ]
      },
      {
        version: 'v4.5',
        date: '2026/07/17',
        title: '称号の装備とゲストモードの追加',
        details: [
          '称号（バッジ）を3つまで選んで、名前の横にセットできるようになりました！',
          'ログインしなくても「ゲストとして遊ぶ」ことができるようになりました！※順位表（ランキング）には参加できません。',
          '順位表（ランキング）の名前の横にレベルや装備した称号アイコンが並ぶようになりました！'
        ]
      },
      {
        version: 'v4.2',
        date: '2026/07/16',
        title: 'リタイアボタンと札のデザイン修正',
        details: [
          'ゲームを途中で諦めてメニューに戻る「リタイア」ボタンを追加しました！',
          '暗いデザインの札の文字を「金色の文字」にして、見やすくかっこよくしました！'
        ]
      },
      {
        version: 'v4.0',
        date: '2026/07/15',
        title: 'ミックスモードのパワーアップ',
        details: [
          'ミックスモード（色々な色を混ぜるモード）で40枚・60枚・80枚のコースが選べるようになりました！',
          'それぞれのコースで1位〜5位に入ると、特別な称号がもらえるようになりました！'
        ]
      }
    ];

    function renderAnnouncements() {
      const container = document.getElementById('makimono-main-content');
      if (!container) return;
      
      container.innerHTML = '';
      
      // Prepend active events notices
      if (window.activeEvents && window.activeEvents.length > 0) {
        window.activeEvents.forEach(ev => {
          const eventBox = document.createElement('div');
          eventBox.style.marginBottom = '0.8rem';
          eventBox.style.padding = '0.6rem 0.8rem';
          eventBox.style.background = '#fef2f2';
          eventBox.style.border = '1px solid #fca5a5';
          eventBox.style.borderRadius = '8px';
          eventBox.style.boxShadow = '0 1px 3px rgba(239, 68, 68, 0.1)';
          
          const escName = typeof window.escapeHTML === 'function' ? window.escapeHTML(ev.title) : ev.title;
          const escRewardName = typeof window.escapeHTML === 'function' ? window.escapeHTML(ev.reward_title_name) : ev.reward_title_name;
          
          eventBox.innerHTML = `
            <div style="font-size: 0.78rem; font-weight: bold; color: #dc2626; display: flex; align-items: center; gap: 4px;">
              <span class="achievement-limited-badge">期間限定</span>
              <span>🎉 【${escName}】開催中！</span>
            </div>
            <div style="font-size: 0.75rem; color: #7f1d1d; margin-top: 4px; line-height: 1.4;">
              期間: <strong>${ev.start_date} ～ ${ev.end_date}</strong><br>
              条件: <strong>クリア時のお手つき回数が一定以下</strong><br>
              （バッジ: ${ev.reward_title_icon} ${escRewardName} がもらえます！）
            </div>
          `;
      ...
      const filtered = showPastAnnouncements ? APP_UPDATES : [APP_UPDATES[0]];
      
      filtered.forEach(up => {
        const updateBox = document.createElement('div');
        updateBox.style.marginBottom = '0.8rem';
        updateBox.style.borderBottom = '1px dashed #cbd5e1';
        updateBox.style.paddingBottom = '0.5rem';
        
        let detailsHTML = up.details.map(d => `<li style="margin-left: 1.2rem; list-style-type: disc;">${d}</li>`).join('');
        
        updateBox.innerHTML = `
          <div style="display: flex; justify-content: space-between; font-weight: bold; color: #92400e; font-size: 0.88rem; margin-bottom: 0.2rem;">
            <span>✨ 【${up.version}】 ${up.title}</span>
            <span style="font-size: 0.72rem; color: #b45309; font-family: monospace;">(${up.date})</span>
          </div>
          <ul style="margin: 0; padding: 0; font-size: 0.82rem; color: #451a03; line-height: 1.5;">
            ${detailsHTML}
          </ul>
        `;
      ...
      const toggleBtn = document.getElementById('makimono-toggle-btn');
      if (toggleBtn) {
        toggleBtn.textContent = showPastAnnouncements ? '最新のおしらせ' : 'かこのおしらせ';
      }
    }
    window.renderAnnouncements = renderAnnouncements;

    function togglePastAnnouncements() {
      showPastAnnouncements = !showPastAnnouncements;
      renderAnnouncements();
    }
    window.togglePastAnnouncements = togglePastAnnouncements;

    function checkVersionUpdate() {
      const lastSeen = goshikiStorage.getItem('goshiki_last_seen_version');
      if (lastSeen !== APP_VERSION) {
        const modal = document.getElementById('version-popup-modal');
        const content = document.getElementById('version-popup-content');
        if (modal && content) {
          const latest = APP_UPDATES[0];
          content.innerHTML = `
            <div style="font-weight: bold; color: #92400e; font-size: 0.95rem; margin-bottom: 0.4rem;">
              🚀 バージョン ${latest.version}： ${latest.title}
            </div>
            <ul style="margin: 0; padding: 0; padding-left: 1rem; line-height: 1.6; font-size: 0.82rem;">
              ${latest.details.map(d => `<li style="margin-bottom: 0.3rem;">${d}</li>`).join('')}
            </ul>
          `;
    ...
    function closeVersionPopup() {
      const modal = document.getElementById('version-popup-modal');
      if (modal) modal.style.display = 'none';
      goshikiStorage.setItem('goshiki_last_seen_version', APP_VERSION);
    }
    window.closeVersionPopup = closeVersionPopup;

    // -------------------------------------------------------------
    // Date & Sorting Rankings Helpers
    // -------------------------------------------------------------
      ...
      try {
        ...
        if (isNaN(d.getTime())) {
          if (typeof timestamp === 'string' && timestamp.length >= 10) {
            return timestamp.substring(0, 10).replace(/-/g, '/');
          }
          return '----/--/--';
        }
        const yyyy = d.getFullYear();
    ...
    function setRankingSort(sortKey) {
      window.rankingSortKey = sortKey;
      
      const timeBtn = document.getElementById('sort-rank-time');
      const dateBtn = document.getElementById('sort-rank-date');
      
      if (timeBtn && dateBtn) {
        if (sortKey === 'time') {
          timeBtn.style.background = 'var(--theme-color)';
          timeBtn.style.color = '#fff';
          timeBtn.style.borderColor = 'var(--theme-color)';
          dateBtn.style.background = '#fff';
          dateBtn.style.color = '#475569';
          dateBtn.style.borderColor = '#cbd5e1';
        } else {
          dateBtn.style.background = 'var(--theme-color)';
          dateBtn.style.color = '#fff';
          dateBtn.style.borderColor = 'var(--theme-color)';
          timeBtn.style.background = '#fff';
          timeBtn.style.color = '#475569';
          timeBtn.style.borderColor = '#cbd5e1';
        }
      }
    ...
    window.setRankingSort = setRankingSort;

    function validateAndResolveTitleIcons(titles) {
      ...
      titles.forEach(t => {
        if (iconCounts[t.icon] > 1) {
          let resolved = false;
          for (let f of fallbacks) {
            const isUsed = titles.some(other => other.icon === f);
            if (!isUsed) {
              console.warn(`[Title Icon Validator] Duplicate icon detected: "${t.icon}" for title "${t.name}". Automatically resolved to "${f}".`);
              iconCounts[t.icon]--;
              t.icon = f;
              iconCounts[f] = 1;
              resolved = true;
    ...
    function getAllTitles() {
      ...
      specials.forEach(t => { titles.push(t); });
      NEW_ACHIEVEMENTS.forEach(t => {
        titles.push({
          key: t.key,
          name: t.name,
          icon: t.icon,
      ...
      const renewals = [
        { key: "hall_of_fame", name: "殿堂入り歌詠み", icon: "殿", req: "自己ベストタイムを更新する", desc: "自分自身の壁を破り、新たな自己新記録を樹立した証！" },
        { key: "top_challenger", name: "頂上の挑戦者", icon: "頂", req: "全国ランキング1位のタイムを更新する", desc: "全国の頂点に立ち、新たな歴代最高記録を塗り替えた英雄！" },
        { key: "yesterday", name: "昨日を超える者", icon: "超", req: "5色それぞれのベストタイムの合計タイムを更新する", desc: "日々努力を重ね、すべての色の合計時間を縮めて過去の自分を超え続けた求道者！" }
      ];
      renewals.forEach(t => { titles.push(t); });

      const eventTitles = [
        { key: "summer_hero", name: "夏祭りの主役", icon: "🏮", req: "夏休みイベント（7/18〜8/24）中にお手つき3回以内でクリアする", desc: "夏休みの期間中にお手つき3回以内でクリアした証！", isLimited: true },
      ...
      return validateAndResolveTitleIcons(titles);
    }
    window.getAllTitles = getAllTitles;

    function updateHeaderUserDisplay() {
      const nameEl = document.getElementById('header-username');
      if (!nameEl) return;

      const logoutOrLoginBtn = document.querySelector('#header-user-widget button');

      if (window.isGuestMode) {
        nameEl.innerHTML = `<span style="color: #64748b; font-weight: normal;">ゲスト（ランキング参加不可）</span>`;
        if (logoutOrLoginBtn) {
          logoutOrLoginBtn.textContent = '登録して保存 🚀';
          logoutOrLoginBtn.style.border = '1px solid #fcd34d';
          logoutOrLoginBtn.style.background = '#fef3c7';
          logoutOrLoginBtn.style.color = '#d97706';
          logoutOrLoginBtn.onclick = function() {
            promptGuestToRegister();
          };
        }
        
        const noticeEl = document.getElementById('settings-guest-login-notice');
        if (noticeEl) noticeEl.style.display = 'flex';
        return;
      }

      if (logoutOrLoginBtn) {
        logoutOrLoginBtn.textContent = 'ログアウト';
        logoutOrLoginBtn.style.border = '1px solid #fecaca';
        logoutOrLoginBtn.style.background = '#fef2f2';
        logoutOrLoginBtn.style.color = '#ef4444';
        logoutOrLoginBtn.onclick = function() {
          handleLogout();
        };
      }
      ...
      let equipped = [];
      try {
        equipped = JSON.parse(goshikiStorage.getItem('goshiki_equipped_titles') || '[]');
      } catch(e) {}

      const allTitles = getAllTitles();
      // Check for limited events titles to toggle wrapper classes
      const wrapperEl = document.getElementById('header-user-wrapper');
      if (wrapperEl) {
        const hasLimitedTitle = equipped.some(key => {
          const t = allTitles.find(item => item.key === key);
          return t && t.isLimited;
        });
        if (hasLimitedTitle) {
          wrapperEl.className = 'header-user-wrapper-limited';
        } else {
          wrapperEl.className = '';
        }
      }
      // Apply level color to header user widget background
      const widget = document.getElementById('header-user-widget');
      if (widget) {
        const xp = parseInt(goshikiStorage.getItem('goshiki_player_xp') || '0');
        const level = typeof window.getLevel === 'function' ? window.getLevel(xp) : 1;
        const lvlStyle = typeof window.getLevelStyle === 'function' ? window.getLevelStyle(level) : { bg: '#f8fafc', border: '#e2e8f0', text: '#475569' };
        
        widget.style.background = lvlStyle.bg;
        widget.style.borderColor = lvlStyle.border;
        
        const userSpan = widget.querySelector('span');
        if (userSpan) {
          userSpan.style.color = lvlStyle.text;
        }
      }
      const iconsHTML = equipped.map(key => {
        const t = allTitles.find(item => item.key === key);
        return t ? `<span style="font-size: 0.95rem; margin-left: 2px;" title="${t.name}">${t.icon}</span>` : '';
      }).join('');

      nameEl.innerHTML = `${escapeHTML(savedName)} <span style="display: inline-flex; align-items: center; vertical-align: middle;">${iconsHTML}</span>`;
    }
    ...
    function renderEquippedSlots() {
      ...
      for (let i = 0; i < 3; i++) {
        const slotEl = document.getElementById(`equipped-slot-${i + 1}`);
        if (!slotEl) continue;
        
        const titleKey = equipped[i];
        if (titleKey) {
          const tInfo = allTitles.find(t => t.key === titleKey);
          if (tInfo) {
            slotEl.innerHTML = `
              <span style="font-size: 1.1rem; line-height: 1.1;">${tInfo.icon}</span>
              <span style="font-size: 0.65rem; font-weight: bold; color: #475569; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 90%; margin-top: 2px;">${tInfo.name}</span>
            `;
            slotEl.style.borderColor = '#8b5cf6';
            slotEl.style.background = '#f5f3ff';
            continue;
          }
        }
        slotEl.innerHTML = `
          <span style="font-size: 1.1rem; color: #cbd5e1; line-height: 1.1;">➕</span>
          <span style="font-size: 0.65rem; color: #94a3b8; font-weight: normal; margin-top: 2px;">（なし）</span>
        `;
        slotEl.style.borderColor = '#e2e8f0';
    ...
    function equipTitle(key) {
      ...
      equipped.push(key);
      goshikiStorage.setItem('goshiki_equipped_titles', JSON.stringify(equipped));
      openAchievementsModal();
    }
    ...
    function openAchievementsModal() {
      ...
      // Render equipped slots widget at the top
      ...
      function getEquipBtnHTML(key, isUnlocked, equippedList) {
        if (!isUnlocked) return '';
        if (equippedList.includes(key)) {
          return `<button disabled style="padding: 0.35rem 0.8rem; font-size: 0.78rem; font-weight: bold; border-radius: 6px; border: 1px solid #c084fc; background: #faf5ff; color: #a855f7; cursor: default; shrink: 0; margin-left: auto;">装備中</button>`;
        }
        return `<button onclick="equipTitle('${key}')" style="padding: 0.35rem 0.8rem; font-size: 0.78rem; font-weight: bold; border-radius: 6px; border: none; cursor: pointer; transition: all 0.2s; shrink: 0; background: #8b5cf6; color: #fff; margin-left: auto;" onmouseover="this.style.background='#7c3aed'" onmouseout="this.style.background='#8b5cf6'">装備</button>`;
      }
      ...
      SECRET_TITLES.forEach(title => {
        ...
        row.innerHTML = `
          ...
          ${getEquipBtnHTML(key, isUnlocked, equipped)}
        `;
        container.appendChild(row);
      });
      ...
      // Render Limited-Time Event achievements
      const eventHeader = document.createElement('div');
      eventHeader.innerHTML = `<h4 style="margin: 1.5rem 0 0.8rem 0; color: #ef4444; font-size: 1.05rem; border-bottom: 2px solid #ef4444; padding-bottom: 0.3rem;">📅 期間限定イベント称号 📅</h4>`;
      container.appendChild(eventHeader);

      const eventTitlesList = [
        { key: "summer_hero", name: "夏祭りの主役", icon: "🏮", req: "夏休みイベント（7/18〜8/24）中にお手つき3回以内でクリアする", desc: "夏休みの期間中にお手つき3回以内でクリアした証！", color: "#f97316" },
        { key: "obon_event", name: "お盆の送り火", icon: "🌌", req: "お盆イベント（8/13〜8/16）中にお手つき2回以内でクリアする", desc: "お盆の期間中にお手つき2回以内でクリアした証！", color: "#6366f1" }
      ];

      eventTitlesList.forEach(badge => {
        const isUnlocked = debugModeEnabled || unlockedBadges.includes(badge.key);
        const row = document.createElement('div');
        row.className = 'achievement-row';
        row.innerHTML = `
          ...
          <div class="achievement-info">
            <div class="achievement-title ${isUnlocked ? 'unlocked' : ''}" style="font-weight: bold; color: ${isUnlocked ? badge.color : '#94a3b8'};">
              ${badge.name} <span class="achievement-limited-badge">期間限定</span> ${isUnlocked ? '✨獲得！✨' : '🔒未獲得'}
            </div>
            <div style="font-size: 0.75rem; color: var(--text-muted);">条件: ${badge.req}</div>
            <div class="achievement-desc ${isUnlocked ? 'unlocked' : ''}" style="font-size: 0.85rem; color: #64748b; margin-top: 0.15rem; ${isUnlocked ? 'display: block;' : 'display: none;'}">${badge.desc}</div>
          </div>
    ...
    window.submitToRanking = function() {
      ...
      let name = (goshikiStorage.getItem('goshiki_current_username') || '').trim();
      name = name.replace(/\s*\(\d+(\.\d+)?秒?\)$/, '').trim();
      if (!name || name === 'ゲスト') {
        alert("お名前を登録してから送信してください。");
        return;
      }
    ...
    window.checkAndUpdateWeeklyHeroes = function(xpGained, missesGained, takenCountGained) {
      // Stub function: rankings are recalculated client-side in real-time from all rankings mode subnodes.
    };

    function getWeeklyBoundaries() {
      const now = new Date();
    ...
    function triggerHeroCardAnimation(cardId) {
      const card = document.getElementById(cardId);
      if (!card) return;
      card.classList.remove('hero-changed-animation');
    ...
    function recalculateAndRenderWeeklyHeroes(snapshots) {
      ...
      const modeCardCounts = {
        blue: 20, pink: 20, yellow: 20, green: 20, orange: 20,
